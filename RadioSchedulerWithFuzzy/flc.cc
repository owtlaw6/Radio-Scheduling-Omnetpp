...
{
    if (!strcmp(msg->getName(),"response")){
        currentDelay = msg->getKind();
    ...
    if (!strcmp(msg->getName(),"start_flc"))
	{
	    ...
	    //int currentDelay = 15;round((double)getParentModule()->getSubmodule("scheduler")->par("meanDelayHP"));

	    cMessage *msgr = new cMessage("poll_sink");
	    send(msgr, "out");

	    int W_HP = 6;//(int)getParentModule()->getSubmodule("scheduler")->par("W_HP");
	    int B = 32;//(int)getParentModule()->getSubmodule("netwrk")->par("B");

	    ev << " currentDelay " << currentDelay << "\n";
	    int new_W_HP = W_HP;
